      ...
      $.ajax({
        ...
        success: function (output) {
          if (output.status === "success") {
            ...
            var deletedRow = $('.delete_data[data-id="' + id + '"]').closest(
              "tr"
            );
            deletedRow.remove();
...
$(document).on("click", ".archive_button", function (e) {
  ...
  var confirmButtonText =
    type === 2 ? "Yes, archive it!" : "Yes, unarchive it!";
  var successMessage =
    type === 2
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location.href =
        "controller/archive-stall.php?stall_slots_id=" + id + "&type=" + type;
    }
  });
});
...
$(document).on("click", ".archive_staff", function (e) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location.href =
        "controller/archive-staff.php?admin_id=" + id + "&type=" + type;
    }
  });
});
